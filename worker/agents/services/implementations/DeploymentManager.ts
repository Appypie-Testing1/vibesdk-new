...
export class DeploymentManager extends BaseAgentService<BaseProjectState> implements IDeploymentManager {
    ...
    private async deploy(params: DeploymentParams): Promise<DeploymentResult> {
        ...
        if (filesToWrite.length > 0) {
            ...
            // Delegate mobile-specific post-write tasks (Metro config, dependency install, .api-url)
            const state = this.getState();
            if (MobileDeploymentHooks.shouldHandle(state)) {
                await MobileDeploymentHooks.onFilesWritten(state, sandboxInstanceId, this.getClient(), this.env, logger);
            }
        }
    ...
    private async createNewInstance(): Promise<BootstrapResponse | null> {
        ...
        // Delegate mobile-specific pre-deploy tasks (app.json patching, .api-url injection)
        if (MobileDeploymentHooks.shouldHandle(state)) {
            await MobileDeploymentHooks.onPreDeploy(state, files, this.env, this.getLog());
        }
     ...
     * Implementations live in @ext/mobile/deployment (sanitizers).
     */
    private static sanitizeFiles<T extends { filePath: string; fileContents: string }>(files: T[], renderMode?: string): T[] {
        ...
        return files.map(file => {
            ...
            if (/^src\/index\.(ts|js)$/.test(file.filePath) && contents.includes('hono')) {
                const sanitized = sanitizeWorkerEntryPoint(contents);
            ...
            if (/wrangler\.jsonc?$/.test(file.filePath) && contents.includes('run_worker_first')) {
                const sanitized = sanitizeWranglerConfig(contents);
                if (sanitized !== contents) {
                    contents = sanitized;
                    changed = true;
                }
            ...
            if (/\.(tsx|jsx)$/.test(file.filePath)) {
                const sanitized = sanitizeJsxBraces(contents);
                if (sanitized !== contents) {
                    contents = sanitized;
                    changed = true;
                }
    ...
    async deployToCloudflare(request?: {
        ...
        // For mobile-fullstack projects, write deployed URL so the dev proxy routes /api/*
        if (deploymentUrl && MobileDeploymentHooks.shouldHandle(state) && state.sandboxInstanceId) {
            try {
    ...
    // ========== EAS BUILD METHODS ==========
    // Implementation lives in @ext/mobile/deployment (EasBuildManager).
    // Thin wrappers kept here for callers that reference deploymentManager methods.

    /**
     * Trigger an EAS build for the given platform.
     * Delegates to EasBuildManager in @ext/mobile/deployment.
     */
    async triggerEasBuild(
        ...
        const deps = {
            stateManager: { getState: () => this.getState(), setState: (s: BaseProjectState) => this.setState(s) },
            getClient: () => this.getClient(),
            getLogger: () => this.getLog(),
            getAgentId: () => this.getAgentId(),
            env: this.env,
        };
        return EasBuildManager.triggerEasBuild(deps, platform, expoToken, callbacks);
    }
}
